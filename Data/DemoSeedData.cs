...
9. Complete the agent's first performance review at the end of their 30-day probationary period.",
        ...
        // ════════════════════════════════════════════════════════════════
        // 17. CS LIVE ALLOCATION SLOTS
        // ════════════════════════════════════════════════════════════════
        var csSlots   = new List<CsLiveAllocationSlot>();
        ...
        if (csAgents.Count >= 2)
        {
            for (int d = 0; d < 5; d++)
            {
                ...
                for (int hour = 8; hour <= 20; hour++)
                {
                    ...
                    csSlots.Add(new CsLiveAllocationSlot
                    {
                        Date        = slotDate,
                        SlotHour    = hour,
                        Agent1Id    = a1.Id,
        ...
        db.CsLiveAllocationSlots.AddRange(csSlots);
        await db.SaveChangesAsync();
        Console.WriteLine($"[DemoSeed] {csSlots.Count} CS Live Allocation slots saved.");
