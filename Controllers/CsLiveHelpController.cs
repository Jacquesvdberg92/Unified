...
public class CsLiveHelpController : Controller
{
    ...
    // ── POST /CsLiveHelp/BroadcastBanner ────────────────────────────────

    [HttpPost]
    [ValidateAntiForgeryToken]
    [Authorize(Roles = $"{Roles.TeamLeader},{Roles.BrandManager},{Roles.SwissArmyKnife}")]
    public async Task<IActionResult> BroadcastBanner(string message)
    {
        ...
        if (trimmed.Length > 300)
            return Json(new { success = false, error = "Message must be 300 characters or fewer." });

        var actor = await _users.GetUserAsync(User);
        var actorName = actor?.DisplayName ?? actor?.UserName ?? "System";
        ...
        await _hub.Clients.Group("cs-board").SendAsync("BroadcastBanner", payload);
        ...
        if (amIds.Count > 0)
            await _hub.Clients.Users(amIds).SendAsync("BroadcastBanner", payload);

        return Json(new { success = true, message = "Broadcast sent." });
    }
}
