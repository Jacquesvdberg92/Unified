...
public class CsLiveAllocationService
{
    private readonly AppDbContext _db;

    public CsLiveAllocationService(AppDbContext db) => _db = db;

    public static IEnumerable<int> SlotHours => Enumerable.Range(8, 13); // 8..20

    // ── Queries ────────────────────────────────────────────────────────────

    public async Task<List<CsLiveAllocationSlot>> GetSlotsForDateAsync(DateTime date)
        => await _db.CsLiveAllocationSlots
            .Include(s => s.Agent1)
            .Include(s => s.Agent2)
    ...
    public async Task<List<CsLiveAllocationSwapLog>> GetSwapLogAsync(DateTime? date = null, int pageSize = 100)
    {
        var q = _db.CsLiveAllocationSwapLogs
            .Include(l => l.Slot)
    ...
    public async Task GenerateScheduleAsync(
        ...
        string createdById)
    {
        var existing = await _db.CsLiveAllocationSlots
            .Where(s => s.Date == date.Date)
            .ToListAsync();

        _db.CsLiveAllocationSlots.RemoveRange(existing);

        foreach (var hour in SlotHours)
        {
            var (a1, a2) = assignments.TryGetValue(hour, out var val) ? val : (null, null);
            _db.CsLiveAllocationSlots.Add(new CsLiveAllocationSlot
            {
                Date        = date.Date,
                SlotHour    = hour,
    ...
    public async Task SwapAgentAsync(
        ...
        string reason)
    {
        var slot = await _db.CsLiveAllocationSlots.FindAsync(slotId)
        ...
        _db.CsLiveAllocationSwapLogs.Add(new CsLiveAllocationSwapLog
        {
            SlotId          = slotId,
